...
##### runs without mpr specification. allows free sigma more or less. as constr only positive. 
...
# # SECTION 1.2: LHC KALMAN FITS
    ...
    #         optim_params,  Xn,_, Pn, se,ll= lhc.run_n_kalmans(t, t_mat_grid, CDS_obs,base_seed = 300,n_restarts=1,MPR=False)
...
#### SECTION 2: CIR MODEL FITS.
    for firm in firms:
        ...
        for X_dim in [3]:# , 2]:
            ...
            np.savez(f"./Results_noMPR/{firm}/Kalman_resultsCIR_Xdim{X_dim}.npz",
                    final_param=params,
                    Xn=Xn,
                    Zn=Zn,
                    Pn = Pn,
...
####### Run alternative LHC kalman with gamma1 as optimization parameter.
    ...
    for firm in firms:
        ...
        for X_dim in [1,2,3]:
            ...
            optim_params,  Xn,_, Pn, se,ll= lhc.run_n_kalmans(t, t_mat_grid, CDS_obs,base_seed = 400,n_restarts=1, MPR=False)
            Xn_kalman,Yn_kalman = lhc.kalman_X_Y(t,Xn)
            # Get reconstructed spreads
            Zn = lhc.CDS_model(t, t_mat_grid,CDS_obs,t,Xn_kalman.T,Yn_kalman)
