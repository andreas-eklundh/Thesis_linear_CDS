...
ax.plot(t, Xn[:,0], "o", alpha=0.7, label=f"Survival Process")
...
# Default intensity: 
fig, ax = plt.subplots(figsize=(10,6))
ax.plot(t, default_intensity , "o", alpha=0.7, label=f"Survival Process")
...
for i in range(1,Xn.shape[1]):
    ...
    ax.plot(t, Xn[:,i], "o", alpha=0.7, label=f"X{i}",color=color)
...
for i in range(Zn.shape[1]):
    ...
    ax.plot(t, CDS_obs[:, i], "-", alpha=0.7,color=color, label=f"Observed {mat_grid[i]}Y CDS")
    ax.plot(t, Zn[:,i].flatten(), "o", linewidth=2,color=color, label=f"Predicdet Kalman, {mat_grid[i]}Y CDS")
ax.set_xlabel("Time (years)")
