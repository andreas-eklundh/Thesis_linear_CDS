...
def psi_prem(lhc, t, t0, t_M):
    ...
    t_grid_len = int(np.floor((t_M - t) / lhc.tenor).item()) + 1
...
@njit
def get_states(lhc, t_obs,t0, T_M_grid, CDS_obs):
    ...
    for time_idx in range(n_obs):
        ...
        for mat_idx in range(n_mat):
            psi_c = psi_cds(lhc,ti, t0i, T_M_grid[mat_idx,time_idx], CDS_obs[time_idx,mat_idx])
            psi_p = psi_prem(lhc,ti, t0i, T_M_grid[mat_idx,time_idx])
...
# ### The simple equation 40 for testing of logic:
# @njit
# def get_states(lhc, t_obs,t0, T_M_grid, CDS_obs):
#     if lhc.m > 1:
#         print("This approach only works for m=1.")
#     # RETHINK THIS A LOT. SEEMS LIKELY THAT THERE IS SOME SORT OF ERROR HERE. 
#     n_obs = len(t_obs)
#     n_mat = T_M_grid.shape[0]

#     # Define initial values
#     X0 = np.ones(shape=(lhc.m,)) * lhc.gamma[0] 
#     X = np.ones((lhc.m, n_obs))
#     Y = np.ones((n_obs)) # Implicitly sets Y0
#     # Time 0 values
#     X[:, 0] = np.array([0.3] * lhc.m)
#     # Previous Z, starting guess
#     Z = np.ones((lhc.m,n_obs))
#     Y_prev = Y[0]
#     X_prev = X[:,0]
...
#         # NOTE: Need a plus here due to parametrization.
#         Y[time_idx] = Y_prev + dt * (lhc.gamma.flatten() @ X_prev)
...
class LHC_single():
    ...
    def rebuild_dynamics(self):
        ...
        # NOTE: A IS DEFINED WITH GAMMA, BUT IN LHC THAT PARAMETRIZATION IS -GAMMA
        # This gamma is then the one for inputting in A
    ...
    def get_states(self,t_obs, T_M_grid, CDS_obs):
        ...
        # New get states functionality:
        # Numba code to generate matrices to solve for.
        X,Y,Z = get_states(lhc, t_obs,t0, T_M_grid, CDS_obs)
    ...
    def optimize_params(self,t_obs, T_M_grid, CDS_obs,penalty=0):
        ...
        if result.success:
            print(f"Optimization succeeded, params {result.x}.")
