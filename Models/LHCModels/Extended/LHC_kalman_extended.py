...
@njit
def cds_derivative(lhc, chi_m1, t,t0, t_mat_grid):
    # Returns
    measure_map = np.zeros((t_mat_grid.shape[0],chi_m1.shape[0]), dtype=np.float64)

    for i in range(t_mat_grid.shape[0]):

        # Pass a scalar from the array X. One for each maturity
        prem = psi_prem(lhc,t,t0,t_mat_grid[i])
        prot = psi_prot(lhc,t,t0,t_mat_grid[i])

        for j in range(chi_m1.shape[0]):
            term1 = prot[j] / (np.dot(prem, chi_m1))
            term2 = np.dot(prot, chi_m1) * prem[j] / (np.dot(prem, chi_m1))**2
            measure_map[i,j] = term1 - term2
...
class LHC_single():
    ...
    def build_constraints(self, m):
        ...
        # g1: x[m] * x[2m] >= 0
        cons.append(
            NonlinearConstraint(lambda x: x[m] * x[2*m], 0, np.inf)
        )

        # g2: -(x[-1] - x[i] + x[i]*x[m+i]) >= 0
        for i in range(m):
            cons.append(
                NonlinearConstraint(lambda x, i=i: -(x[-1] - x[i] + x[i]*x[m+i]),
                                    0, np.inf)
            )

        # Bounds handle nonnegativity more cleanly than separate constraints
        bounds = Bounds([0.0]*(2*m+1), [np.inf]*(2*m+1))
    ...
    def optimize_params(self,t_obs, T_M_grid, CDS_obs,penalty=0):
        ...
        # cons, bounds = self.build_constraints(self.m)
        ...
        #     bounds=bounds,
        #     constraints=cons,
        #     options={'xtol' : 1e-6, 'gtol' : 1e-4, 'barrier_tol': 1e-8, 'verbose': 3}
        # )
...
######################### KALMAN FILTER SECTION #########################
    def build_P_params(self,params, n_mat):
        ...
        if params is None:
            ...
            # Assume that homogenous variance for each process
            sigma_i = rng.uniform(0.01, 0.09, size=(X_dim,))       # Kappa given
            # sigma_i = rng.uniform(0.01, 0.09, size=(Y_dim,))       # Kappa given
        ...
        # asumme structure is the same (TODO, check this. Likely not)
            ...
            # sigma_i,sigma_err = params[2*self.m:3*self.m-1], params[-1]
    ...
    def build_constraints_kalman(self, m):
        cons = []

        # product constraints
        cons.append(
            NonlinearConstraint(lambda x: x[m-1] * x[2*m-1], 0, np.inf)
        )
        cons.append(
            NonlinearConstraint(lambda x: x[2*m+1] * x[3*m+1], 0, np.inf)
        )

        # g2 constraints: -(x[-1] - x[i] + x[i]*x[m+i]) >= 0
        for i in range(m):
            cons.append(
                NonlinearConstraint(lambda x, i=i: -(x[-1] - x[i] + x[i]*x[m+i]),
                                    0, np.inf)
    ...
    def get_kalman_params(self,t_obs, T_M_grid, CDS_obs,p):
        ...
        # bounds = Bounds([0]*x0, [np.inf]*x0)

        # constraints = self.build_constraints_kalman(self.m)

        # res = minimize(
        #     fun=self.kalmanfilter,
        #     x0=x0,
        ...
        res = minimize(
            fun=self.kalmanfilter,
            x0=x0,
            args=(t_obs, t0, T_M_grid, CDS_obs,p),
            method='Nelder-Mead',
        )
        # Then ready to optimize
        optim_params = res.x

        Xn,pred_Zn,Pn = self.kalmanfilter(optim_params,t_obs,t0,T_M_grid,
                                                                 CDS_obs,p,get_result=True)
        return optim_params, Xn,pred_Zn,Pn


    # One kalman filter for optimizing and one for outputting.
    def kalmanfilter(self, params, t_obs,t0,T_M_grid,CDS_obs, p,get_result=False):
        print(params)
        ...
        A_trans,Qt,R = self.build_matrices(lhc_p,sigma,sigma_err,Delta,n_mat)
        ...
        # Initial guess for cov of X
        P_state = np.sqrt(np.maximum(P_state,1e-6))
        P0 = (Qt @ np.diag(P_state)) @ (Qt @ np.diag(P_state)).T
        ...
        # Set initial values equal to Y and P to initial quess.
        # Xn[n,:], Pn[n,:,:] = pred_Xn, P0

        # Get initial Extended kalman sizes.
        cds_prime = cds_derivative(lhc_q,chi_0,t_obs[0],t0[0],T_M_grid[:,0])
        cds_model = cds_fun(lhc_q,chi_0,t_obs[0],t0[0],T_M_grid[:,0])
        ...
        for n in range(0,n_obs):
            # UPDATE STEP
            Xn[n,:], Pn[n,:,:],v_n,S_n = self.update_step(pred_Xn,pred_Pn,C,d,R,CDS_obs[n,:])
            # Update log likelihood.

            det_S = np.abs(np.linalg.det(S_n))
            try:
                S_inv = np.linalg.inv(S_n)
            except:
                S_inv = np.linalg.pinv(S_n)
            log_likelihood += - 0.5 * (S_n.shape[0] * np.log(2*np.pi) + np.log(det_S) +
                                       v_n.T @ S_inv @ v_n
            )
            ...
            P_state = np.array([Xn[n,i] * (Xn[n,0] - Xn[n,i]) for i in range(1,L) ])
            P_state = np.sqrt(np.maximum(P_state,1e-6))
            ...
            if (n < n_obs - 1): # Not sensible to predict further.
                ...
                A = expm(A_trans)
                b = np.zeros(A_trans.shape[0])
                pred_Xn, pred_Pn = self.prediction_step(Xn[n,:], Pn[n,:,:],A,b,Qt)

                # Store predictions just to see outcome
                pred_Zn[n,:] = C @ Xn[n,:] + d

        if get_result is True:
            return Xn, pred_Zn, Pn
        else:
            # Penalise if constraints are not satisfies.
            penalty_func = 0
            # enforce constraint (1)
            g1 = params[self.m-1]*params[2*self.m-1]
            penalty_func +=  max(0, -g1)**2
            g2 = params[2*self.m+1]*params[3*self.m+1]
            penalty_func +=  max(0, -g2)**2
            for i in range(self.m):
            # enforce constraint (2)
                g3 = params[2*self.m] - params[i] + params[i]*params[self.m+i]
                penalty_func +=  max(0, g3)**2
            for i in range(2*self.m+1, 3*self.m+1):
                g4 = params[2*self.m] - params[i] + params[i]*params[self.m+i]
                penalty_func +=  max(0, g4)**2

            for i in range(len(params)):
                penalty_func +=  max(0, -params[i])**2


            # Non-negativity constraints (3)
            log_likelihood_mod = log_likelihood + p * penalty_func


            return - log_likelihood_mod



    def update_step(self,pred_Xn,pred_Pn,C,d,R,cds_obs):
        # Get prediction error
        v_n = cds_obs - (C @ pred_Xn + d)
        # Error covariance
        S_n = C @ pred_Pn @ C.T + R
        # Kalman Gain:
        try:
            K_n = pred_Pn @ C.T @ np.linalg.inv(S_n)
        except:
            # Fallback if not feasible
            K_n = pred_Pn @ C.T @ np.linalg.pinv(S_n)
        # Recalculate the states.
        Xn = pred_Xn + K_n @ v_n
        Pn = pred_Pn - K_n @ S_n @K_n.T

        return Xn,Pn,v_n,S_n

    def prediction_step(self,Xn,Pn,A,b,Q):
        pred_Xn = A @ Xn + b
        pred_Pn = A @ Pn @ A.T + Q

        return pred_Xn, pred_Pn
