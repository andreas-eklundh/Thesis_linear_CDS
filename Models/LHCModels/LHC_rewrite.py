...
def build_P_params(params,params_q,lhc_p):
        ...
        theta_p = (lambda_i + kappa*theta) / (kappa-lambda_i)
        sigma_i,sigma_err = params[m:m], params[-1]
...
def nonlinear_constraints( params, m):
    ...
    # g1 = (lambda_i[-1] + theta[-1] * kappa[-1])  #- 0.5 * (sigma[-1]**2)
    # cons.append(g1)

    ### Note, the constraints on theta_p and kappa_p still needs to hold...
    for i in range(m):
        g1 = np.minimum( theta[i] * kappa[i],lambda_i[i] + theta[i] * kappa[i])  - 0.5 * (sigma[-1]**2)
        cons.append(g1)
    ...
    # g3 <= 0 -> -g3 >= 0
    for i in range(m):
        g3 = gamma1 - (kappa[i] - lambda_i[i]) + (lambda_i[i] + kappa[i] * theta[i]) + 0.5 * (sigma[i]**2)
        cons.append(-g3)
...
def drift_deriv_term(Xn, lhc_p, Delta):
    ...
    def build_P_params(self,params=None, gamma1=None,rng=None):
        ...
        if params is None:
            ...
            # Set inital values. Need to comply with (38)
            # lambda_i = np.ones(X_dim)*0.1  # initalise at some values
            lambda_i = rng.uniform(1e-3,self.kappa - 0.05,size=(X_dim,)) # initialise gamma to comply.
            kappa =  self.kappa - lambda_i  #rng.uniform(gamma1, 0.99, size=(X_dim,))       # Kappa given 
            theta = (lambda_i + self.kappa*self.theta ) / (self.kappa - lambda_i)
            ...
            sigma_err = rng.uniform(0.001, 0.05, size=(Y_dim,))       # Kappa given 
        ...
        self.lambda_i,self.theta_p, self.kappa = lambda_i, theta,kappa
    ...
    def get_kalman_params(self, t_obs, T_M_grid, CDS_obs, x0, lhc_p,lhc_q,base_seed):
        ...
        result = differential_evolution(
            ...
            strategy='best1bin',
            popsize=4,         # larger popsize -> more exploration
        ...
        if self.kalman_obj >= 1e12:
            ...
            kappa, theta, gamma1 = params_q[:m],params_q[m:2*m], params_q[-1]
            kappa_p  = kappa - lambda_i
            theta_p = (lambda_i + kappa*theta) / kappa_p
            lhc_p = rebuild_lhc_struct(kappa_p, theta_p, gamma1, r, Y_dim, delta, tenor)
            lhc_q = rebuild_lhc_struct(kappa, theta, gamma1, r, Y_dim, delta, tenor)
