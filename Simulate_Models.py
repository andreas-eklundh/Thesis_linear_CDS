...
for i in range(1, n_states):
    ...
    ax.plot(T_path, X[i-1, :], "-", alpha=0.9, label=f"{state_name} (Filipovic)", color="gold")
...
fig.tight_layout()
fig.savefig(os.path.join(save_path, "SimulLHC_LatentStates_Combined.png"), dpi=150)
plt.close(fig)
...
fig.savefig(os.path.join(save_path, "True_Simul_CIR.png"), dpi=150)
plt.close(fig)
