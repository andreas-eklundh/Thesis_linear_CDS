...
if __name__ == '__main__':
    ...
    for t_idx in range(CDS_cir.shape[0]):
        ...
        print(f'Done with {(t_idx+1) / CDS_cir.shape[0]}')
    ...
    params_cir_kalman , Xn,Zn,Pn,se = cir.run_kalman_filter(T_return, t_mat_grid, Gamma_kalman_noise)
    ...
    for X_dim in [1,2,3]:
        ...
        results_df = pd.DataFrame(results)
        results_df.to_csv(f"./Likelihoods/likelihood_surface_scan_dual_X{X_dim}.csv", index=False)
        sns.set(style="whitegrid", font_scale=1.2)
