...
if __name__ == '__main__':
    ...
    mu1 = lhc.solve_mu1(lhc.kappa_p,lhc.theta_p,lhc.gamma1)
    ...
    T_path, chi_Q = lhc.simul_latent_states(chi0=chi0,T=T,M=M,n_mat=n_mat,seed=3000,scheme='Euler',
                                            measure='P')
    ...
    ### Run one kalman filter to see identification...
    lhc_kalman_params,  _,_, _= lhc.run_n_kalmans(T_path, t_mat_grid, CDS_simul_actual.T,base_seed=2000,n_restarts=1)
    ...
    def build_grid(center_params, label):
        ...
        for key, val in center_params.items():
            ...
            if key == "lambda_i":
                ...
                grids[key] = np.linspace(lowp_b, up_b / 2, n_points)
                ...
                sigma_thetakap = np.sqrt(2 * (center_params["kappa"]**2 + center_params["lambda_i"]))
