...
if __name__ == "__main__":
    ...
    # Reference no gemeni for this code piece.
    ...
    def generate_latex_table(data, metrics):
        ...
        for firm, dimensions in data.items():
            ...
            # Assuming you want to print all dimensions one after another for the firm
    ...
    #         cir = CIRIntensity(r=0.00248,delta=0.4,tenor=0.25,X_dim=i,cascading=True)
    #         se = cir.kalman_SE(final_paramCIR,t,t_mat_grid,Gamma_kalman_scale,result=True,eps=1e-7)

    #         # # Try rerun to get SE and see if same.
    #         # lhc = LHC_single(r=0.00248,delta=0.4,cds_tenor=0.25)
    #         # lhc.initialise_LHC(Y_dim=1,X_dim=i,X0=np.array([0.5]*i))
    #         # params_p =  final_paramLHCK[2*lhc.m:]
    #         # params_q = final_paramLHCK[:2*lhc.m]
    #         # lambda_i = params_p[:lhc.m]
    #         # kappa, theta = params_q[:lhc.m],params_q[lhc.m:2*lhc.m]
    #         # gamma1 = lhc.calc_gamma1(kappa,theta)
    ...
    #         # lhc_p = rebuild_lhc_struct(kappa_p, theta_p, gamma1[0], lhc.r,
    #         #                             lhc.Y_dim, lhc.delta, lhc.tenor,lambda_i=None)
    #         # lhc_q = rebuild_lhc_struct(kappa, theta, gamma1[0], lhc.r,
    #         #                             lhc.Y_dim,lhc.delta, lhc.tenor,lambda_i=None)

    #         # lhc.build_P_params(params_p,gamma1)
    #         # lhc.flatten_params()
    #         # # Add gamma1 to params.
    #         # params_q = np.append(params_q,gamma1)
    #         # lhc.unflatten_params(params_q)
    #         # # Get SE
    #         # kalman_args = (t,t,t_mat_grid,CDS_obs,lhc_p,lhc_q,lhc.X0)
    #         # run_params = np.append(final_paramLHCK[:2*lhc.m],final_paramLHCK[2*lhc.m+1:])
    #         # se = lhc.kalman_SE(params_opt=run_params,f_args=kalman_args,eps = 1e-9)
    ...
    for firm in firms:
        ...
        for i in X_dims:
            ...
            # Plot dir.
            directory = f"C:/Users/andre/OneDrive/KU, MAT-OEK/Kandidat/Thesis/Thesis_linear_CDS/Results/Chapter7"
            ...
            # Loop through maturities and make a separate plot for each
            ax.plot(t, ZnLHCK[:,2], "-", alpha=0.7, color='blue', label="LHC Kalman")
            ax.plot(t, CDS_LHC[:,2], "-", alpha=0.7, color='red', label="LHC Filipovic")
            ax.plot(t, CDS_cirCIR[:,2], "-", alpha=0.7, color='green', label="CIR Kalman")
            ax.plot(t, CDS_obs[:,2], "o", alpha=0.5, color='black', label="Observations")

            ax.grid(True)
            ax.set_xlabel("Time (years)")
            ax.set_ylabel("Spreads")
